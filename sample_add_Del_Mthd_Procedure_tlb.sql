--####Insert DELIVERY METHOD into PCORNET [PROCEDURES] table #### 
with patient_pop as (
	SELECT 
...
INSERT INTO [AMMI].[dbo].[PROCEDURES]([PROCEDURESID] ,[PATID] ,[ENCOUNTERID] ,[ENC_TYPE] ,[ADMIT_DATE]
	...
	select 
	 ROW_NUMBER() OVER (order by (select 1))+ (SELECT COALESCE(MAX(PROCEDURESID),0) FROM [dbo].[PROCEDURES]), 
	 e.PATID
	 ...
	 --PX
	 ,  CASE
			when  a.[DELIVERY_METHOD] = 'VBAC, Spontaneous'  then  'Unknown'
			when  a.[DELIVERY_METHOD] = 'Vaginal, Spontaneous'  then  '48204000'
			when  a.[DELIVERY_METHOD] = 'C-Section, Classical'  then  '84195007'
			when  a.[DELIVERY_METHOD] = 'Vaginal, Breech'  then  '177157003'
			when  a.[DELIVERY_METHOD] = 'C-Section, Low Vertical'  then  '11466000'
			when  a.[DELIVERY_METHOD] = 'C-Section, Unspecified'  then  '11466000'
			when  a.[DELIVERY_METHOD] = 'C-Section, Extraperitoneal'  then  'Unknown'
			when  a.[DELIVERY_METHOD] = 'Vaginal, Forceps'  then  '302383004'
			when  a.[DELIVERY_METHOD] = 'Vaginal, Vacuum (Extractor)'  then  '61586001'
			when  a.[DELIVERY_METHOD] = 'C-Section, Other Specified Type'  then  'Unknown'
			when  a.[DELIVERY_METHOD] = 'Other'  then  'Unknown'
			when  a.[DELIVERY_METHOD] = 'Induction'  then  'Unknown'
			when  a.[DELIVERY_METHOD] = 'C-Section, Low Transverse'  then  '11466000'
			when a.[DELIVERY_METHOD]  is NULL then 'Unknown'
	   END
